...
from s2cnn import SO3Integration
...
def load_data(path):
    ...
    train_data = dataset["train"]["images"][:, None, :, :].astype(np.float32)
    train_labels = dataset["train"]["labels"].astype(np.int64)

    # TODO normalize dataset
    # mean = train_data.mean()
    # stdv = train_data.std()

    test_data = dataset["test"]["images"][:, None, :, :].astype(np.float32)
    test_labels = dataset["test"]["labels"].astype(np.int64)

    return train_data[::60], train_labels[::60], test_data, test_labels
